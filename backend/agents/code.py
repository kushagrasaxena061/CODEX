...
async def implement_step(step_description: str, agent_role: str, files_context: dict, workspace_root: str, history: str = "") -> AgentMessage:
    ...
    system_prompt = (
        f"You are an Expert {agent_role}. You can write code, create files, and run commands.\n"
        ...
        "}\n"
        "CRITICAL RULES:\n"
        "1. TO CREATE A NEW FILE: Provide the exact `file_path`, leave `search_block` completely empty (\"\"), and put the full code in `replace_block`.\n"
        "2. Fulfill your specific step completely. If asked to write a frontend button that changes text, write the full HTML/JavaScript logic to do it."
    )
    ...
    try:
        ...
        return AgentMessage(agent_name=agent_role, status="failed", message=f"Parse Error: {e}")
